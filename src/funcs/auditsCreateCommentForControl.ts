 ...
 */
export function auditsCreateCommentForControl(
  client: VantaCore,
  request: operations.CreateCommentForControlRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AuditControlComment,
    | VantaError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateCommentForControlRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AddAuditControlCommentInput, {
    explode: true,
  });
  ...
  const path = pathToFunc("/audits/{auditId}/controls/{controlId}/comments")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AuditControlComment$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
