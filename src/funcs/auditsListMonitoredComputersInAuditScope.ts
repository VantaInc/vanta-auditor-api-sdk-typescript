 ...
 */
export function auditsListMonitoredComputersInAuditScope(
  client: VantaCore,
  request: operations.ListMonitoredComputersInAuditScopeRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.PaginatedResponseMonitoredComputer,
    | VantaError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListMonitoredComputersInAuditScopeRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/audits/{auditId}/monitored-computers")(pathParams);

  const query = encodeFormQuery({
    "complianceStatusFilterMatchesAny":
      payload.complianceStatusFilterMatchesAny,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PaginatedResponseMonitoredComputer$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
