 ...
 */
export function auditsGetInformationRequestEvidence(
  client: VantaCore,
  request: operations.GetInformationRequestEvidenceRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.InformationRequestEvidence,
    | VantaError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetInformationRequestEvidenceRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.InformationRequestEvidence$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
