...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateInformationRequestRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.CreateInformationRequestInput, {
    explode: true,
  });
  ...
  const path = pathToFunc("/audits/{auditId}/information-requests")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.InformationRequest$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
