 ...
 */
export function auditsGetOrganizationNotifications(
  client: VantaCore,
  request: operations.GetOrganizationNotificationsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AuditOrganizationNotifications,
    | VantaError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetOrganizationNotificationsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/audits/{auditId}/organization/notifications")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AuditOrganizationNotifications$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
