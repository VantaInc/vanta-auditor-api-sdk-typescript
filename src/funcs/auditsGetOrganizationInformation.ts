 ...
 */
export function auditsGetOrganizationInformation(
  client: VantaCore,
  request: operations.GetOrganizationInformationRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AuditOrganizationInformation,
    | VantaError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetOrganizationInformationRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/audits/{auditId}/organization/information")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AuditOrganizationInformation$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
