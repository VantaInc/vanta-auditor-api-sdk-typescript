...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetFrameworkCodesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/audits/{auditId}/framework-codes")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.FrameworkCodes$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
