 ...
 */
export function auditsListCommentsForControl(
  client: VantaCore,
  request: operations.ListCommentsForControlRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.PaginatedResponseAuditControlComment,
    | VantaError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListCommentsForControlRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PaginatedResponseAuditControlComment$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
